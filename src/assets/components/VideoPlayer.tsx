...
export const VideoPlayer = (url: Props) => {
    ...
    const PlayerComponents = {
        ...
        containerPlayer: useRef<HTMLDivElement>(null),
        ContainerControls: useRef<HTMLDivElement>(null),
        TopControls: useRef<HTMLDivElement>(null),
        AgeGroupControls: useRef<HTMLDivElement>(null),
        MiddleControls: useRef<HTMLDivElement>(null),
        BottomControls: useRef<HTMLDivElement>(null),
    }

    useEffect(() => {
        if(StatusVideoPlayPause){
            PlayerComponents.Player.current?.play();
        }else{
            PlayerComponents.Player.current?.pause();
        }
    }, [StatusVideoPlayPause]);
    
    PlayerComponents.Player.current?.addEventListener('canplaythrough ', (ev: Event) => {
        console.log('canplaythrough ', ev);
    })

    return(
        <VideoContainer>
            <VideoProtection id="player" ref={PlayerComponents.containerPlayer}>
                <video
                    preload="auto"
                    ref={PlayerComponents.Player} 
                    src={url.url}
                    muted
                    poster="https://c4.wallpaperflare.com/wallpaper/557/944/965/xilmo-anime-container-ship-workers-hd-wallpaper-preview.jpg"
                ></video>
                <UserView 
                    ref={PlayerComponents.ContainerControls}
                    onClick={(ev) => {
                        if(ev.target == PlayerComponents.ContainerControls.current || ev.target == PlayerComponents.MiddleControls.current || ev.target == PlayerComponents.MiddleControls.current || ev.target == PlayerComponents.BottomControls.current || ev.target == PlayerComponents.TopControls.current || ev.target == PlayerComponents.AgeGroupControls.current){
                            SetStatusVideoPlayPause(!StatusVideoPlayPause);
                        }
                }}>
                    <TopControls>
                        <TopAllControls ref={PlayerComponents.TopControls}>
                            <button>
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    if(IsFullScreen){
                                        PlayerComponents.containerPlayer.current?.requestFullscreen();
                                    }else{
                    ...
                    <MiddleControls ref={PlayerComponents.MiddleControls}>
                        <button>
                            <svg width="800px" height="800px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <BottomControls ref={PlayerComponents.BottomControls}>
                        <BottomTimers>
                            <div>00:00</div>
                            <div>24:50</div>
                        </BottomTimers>
...
const TopAllControls = styled.div`
    ...
    @media only screen and (max-width: 690px){
        padding: var(--padding-less-video-controls);
